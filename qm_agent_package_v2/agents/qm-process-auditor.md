...
모든 Finding(부적합)에는 분기 단위로 **연속 추적 가능한 ID**를 부여합니다. qm-capa-tracker와 동일한 NC- 접두사 체계를 공유하여, 감사에서 발견된 부적합이 그대로 CAPA 추적으로 이어질 수 있습니다.
...
**생성 규칙:**
1. **YYYYQQ**: 감사 작성일이 속한 연·분기 (예: 2026Q2 = 4~6월)
2. **NNN**: 해당 분기 내 직전 보고 마지막 순번 +1, 3자리 zero-padded
3. 분기가 바뀌면 `001`부터 다시 시작
4. **qm-capa-tracker와 순번 공유**: 같은 분기에서 capa-tracker가 `NC-2026Q2-005`까지 발급했다면, 감사는 `NC-2026Q2-006`부터 시작

**ID 발급 시 사전 확인 + 발급 직후 동기화:**
- 사전: 직전 보고 로그(`~/.claude/qm-state/qm-report-log.json` 또는 사용자 제공 자료)에서 해당 분기 마지막 NC 순번 조회 → +1
- 로그 부재 시 사용자에게 "이 분기 첫 보고인지" 확인 후 `001`부터 시작
- **발급 직후 동기화 (필수):** 새 Finding을 발급하면 즉시 `qm-report-log.json`의 `id_counters[YYYYQQ].NC` 값을 발급 직후 카운트로 갱신. 이 동기화가 누락되면 다음 감사 또는 qm-capa-tracker 호출에서 NC ID 중복 위험.
- 감사 완료 후 발급한 Finding ID 목록을 `reports[].issued_ids` 배열에도 추가 (qm-exec-reporter가 보고 작성 시 일괄 처리하거나, 단독 호출 시 본 에이전트가 직접 추가)

**효과:**
- 감사 Finding → CAPA → 효과성 검증을 ID로 직접 연결
- 외부 부서/경영진 보고 시 "이번 분기 NC 5건 중 2건은 CA 진행 중" 같은 짧은 참조 가능
- 분기 간 동일 영역 재발 자동 식별
...
|------|------|------------------|
| **Major** | QMS의 부재 또는 완전한 붕괴 | 전체 TC 작성 누락, 회귀 테스트 미실시, 검증 없이 마일스톤 통과 |
