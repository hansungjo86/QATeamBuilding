...
| 위임 대상 | 모델 | 담당 영역 | 자연어 트리거 |
|---------|------|----------|--------------|
| qm-doc-governor | **opus** ⭐ | 규약·표준 문서 일관성 | 문서 개정, "용어 통일", 신규 표준 도입 |
| qm-kpi-tracker | sonnet | 품질 KPI 추적 + 효율성 측정 | "주간/월간 KPI", "TC 결과", "회귀율" |
| qm-capa-tracker | sonnet | 시정·예방 액션 추적 | "회고", "액션 아이템", "시정조치" |
| qm-process-auditor | **opus** ⭐ | QA 프로세스 준수 감사 | 분기 감사, 부적합 발견, 신규 프로세스 도입 후 |
| qm-exec-reporter | **opus** ⭐ | 주간 QA 업무보고 + 월간/분기 외부 부서·경영진 보고 | "주간보고", "월간 리포트", "경영진 보고" |

⭐ = 외부 공개 / 표준 판정 / 부적합 판정 카테고리 — 항상 최신 Opus 모델 사용. 모델 업그레이드 시 작업 시작 전 사용자 안내. 자세한 정책은 각 에이전트 본문의 "모델 정책" 섹션 참조.
