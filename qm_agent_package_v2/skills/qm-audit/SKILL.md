...
> 표준 출처: 사용자 GitHub QA 프로세스 2-3절. TC 자동 생성 도구를 사용 중이라면 그 도구의 표준(예: TC Team v2 skills)도 추가 기준.

체크포인트:
- TC가 정의된 절차에 따라 작성되었는가 (수동 작성 / 자동 생성 도구 모두 해당)
- 자동 생성 도구를 사용 중이라면 그 파이프라인이 실행되었는가
- 4가지 검토 항목이 점검되었는가
  - 기획서 주요 기능 빠짐없이 반영
  - 정상/부정/예외 비율 적절
  - 취소선 처리된 기능 제외
