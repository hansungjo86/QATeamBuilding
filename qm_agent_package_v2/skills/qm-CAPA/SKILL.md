...
| 구분 | 시점 | 대상 | 사례 |
|------|------|------|------|
| **시정 (Correction)** | 사후 즉시 | 발생한 사건 자체 | 빌드 결함 긴급 수정 (마일스톤 직전) |
...
| 분류 | 정의 | MMORPG 환경 예시 |
|------|------|------------------|
| Major | QMS 부재 또는 완전한 붕괴 | 전체 TC 작성 누락, 회귀 테스트 미실시, 검증 없이 마일스톤 통과 |
...
```
문제: 빌드 직전 보스 몬스터 데이터 오류 발견 (긴급 수정 필요)
...
- **환경 (Environment)**: 빌드 환경 변화, 외주 자산 변경, 외부 의존성
