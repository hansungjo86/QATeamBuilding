...
```
@qm-team-v1 이번 분기 QM 종합 보고서 만들어줘
```
...
```
주간보고 작성해줘
→ qm-exec-reporter로 자동 위임 (주간 워크플로우 — Confluence 회의록 트리에 페이지 생성/업로드, 업로드 직전 사용자 동의 필수)
```
...
A. 에이전트 파일명에 한글이 포함되어 있어 일부 환경에서 문제가 될 수 있습니다. 필요 시 영문으로 변경 가능 (예: `qm-team-v1.md` → `qm-team-v1.md`). 단, 변경 시 다른 에이전트에서의 참조도 함께 변경해야 합니다.
